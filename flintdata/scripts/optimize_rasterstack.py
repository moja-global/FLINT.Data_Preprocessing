...
@click.option(
    "-n", "--raster-name", type=str, required=True, help="Name of output stack"
)
@click.option(
    "-o",
    "--output-folder",
...
def optimize_rasterstack(
    ...
    with contextlib.ExitStack() as outer_env:
        ...
        with contextlib.ExitStack() as file_env, warnings.catch_warnings():
            try:
                rasters = [
                    file_env.enter_context(rasterio.open(f, "r"))
            ...
            for tile in flintdata.flinttile.tiles(west, south, east, north):
                with contextlib.ExitStack() as es, warnings.catch_warnings():
                    ...
                    dst_width = max(
                        1, round((tile_bounds[2] - tile_bounds[0]) / dst_res[0])
                    )
                    dst_height = max(
                        1, round((tile_bounds[3] - tile_bounds[1]) / dst_res[1])
                    )
                    ...
                    for layer in rasters:
                        ...
                        with warnings.catch_warnings():
                            ...
                            tile_data = vrt.read(
                                ...
                                resampling=rs_method,
                                window=out_window,
                                out_shape=(dst_width, dst_height),
                            )
                    ...
                    for row in range(0, dst_height, block_height):
                        for col in range(0, dst_width, block_width):
                            block = tile_stack[
                                col: col + block_width, row: row + block_height, :
                            ]
